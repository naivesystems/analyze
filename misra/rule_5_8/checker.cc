...
void ReportError(string kind, string name, string loc, string other_loc,
  ...
  pb_result->set_error_kind(
      analyzer::proto::Result_ErrorKind_MISRA_C_2012_RULE_5_8);
...
class ExternalVDCallback : public MatchFinder::MatchCallback {
 public:
  void Init(ResultsList* results_list,
            unordered_map<string, string>* external_name_locations,
            unordered_map<string, string>* non_external_name_locations,
            MatchFinder* finder) {
    results_list_ = results_list;
    external_name_locations_ = external_name_locations;
    non_external_name_locations_ = non_external_name_locations;
  ...
  void run(const MatchFinder::MatchResult& result) override {
    ...
    auto external_it = external_name_locations_->find(name);
    auto non_external_it = non_external_name_locations_->find(name);
    if (vd->hasExternalFormalLinkage()) {
      if (external_it == external_name_locations_->end()) {
        external_name_locations_->emplace(name, loc);
      } else if (loc != external_it->second) {
        ReportError("variable", name, loc, external_it->second, path,
                    line_number, results_list_);
      }
      if (non_external_it != non_external_name_locations_->end() &&
          loc != non_external_it->second) {
        ReportError("variable", name, loc, non_external_it->second, path,
                    line_number, results_list_);
      }
    } else {
      if (external_it != external_name_locations_->end() &&
          loc != external_it->second) {
        ReportError("variable", name, loc, external_it->second, path,
                    line_number, results_list_);
      }
      if (non_external_it == non_external_name_locations_->end()) {
        non_external_name_locations_->emplace(name, loc);
      }
    }
  }

 private:
  unordered_map<string, string>* external_name_locations_;
  unordered_map<string, string>* non_external_name_locations_;
 ...
 public:
  ...
  void run(const MatchFinder::MatchResult& result) override {
    ...
    if (fd->hasExternalFormalLinkage()) {
      if (external_it != external_name_locations_->end() &&
          loc != external_it->second) {
        ReportError("function", name, loc, external_it->second, path,
                    line_number, results_list_);
      } else {
        external_name_locations_->emplace(name, loc);
      }
      if (non_external_it != non_external_name_locations_->end() &&
          loc != non_external_it->second) {
        ReportError("function", name, loc, non_external_it->second, path,
                    line_number, results_list_);
      }
    } else {
      if (external_it != external_name_locations_->end() &&
...
void Checker::Init(ResultsList* results_list) {
  ...
  external_vd_callback_->Init(results_list_, &external_name_locations_,
                              &non_external_name_locations_, &finder_);
  external_fd_callback_ = new ExternalFDCallback;
  external_fd_callback_->Init(results_list_, &external_name_locations_,
                              &non_external_name_locations_, &finder_);
}
