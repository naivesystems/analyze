...
namespace misra {
namespace rule_8_6 {
int rule_8_6(int argc, char** argv) {
  ...
  misra::rule_8_6::Checker checker(&result_list);
  int status =
      tool.run(newFrontendActionFactory(checker.GetMatchFinder()).get());
  LOG(INFO) << "libtooling status: " << status;
  if (misra::proto_util::GenerateProtoFile(result_list, results_path).ok()) {
    LOG(INFO)
        << "rule 8.6 checker has finished, write results to '/output/results'";
  }
  return 0;
}
}  // namespace rule_8_6
...
podman_image::bigmain::SuffixRule _("misra/rule_8_6",
                                    misra::rule_8_6::rule_8_6);
