...
namespace simplecpp {
    ...
    public:
        ...
        Location(const Location &loc) : files(loc.files), fileIndex(loc.fileIndex), line(loc.line), col(loc.col), lineDirectiveStack{loc.lineDirectiveStack} {}
        ...
        Location &operator=(const Location &other) {
            if (this != &other) {
                ...
                line = other.line;
                col  = other.col;
                lineDirectiveStack = other.lineDirectiveStack;
            }
            return *this;
        }
        ...
        const void pushLineDirective(unsigned int starting_line_number) {
            lineDirectiveStack.push_back({this->fileIndex, {this->line, starting_line_number}});
        }

        const std::pair<unsigned int, unsigned int> revealRealFileIndexAndLine() const {
            auto lineDirectiveStack = this->lineDirectiveStack;
            auto fileIndex = this->fileIndex;
            auto line = this->line;
            while(!lineDirectiveStack.empty()) {
                auto lineDirective = lineDirectiveStack.back(); lineDirectiveStack.pop_back();
                fileIndex = lineDirective.first;
                // For `A.c:B #line C D.c`, the real line number should be: current line number - C + B
                line = line - lineDirective.second.second + lineDirective.second.first;
            }
            return {fileIndex, line};
        }

        // For `A.c:B #line C D.c`, the stack stores <A.c,<B,C>>
        std::vector<std::pair<unsigned int, std::pair<unsigned int,unsigned int>>> lineDirectiveStack;
    private:
