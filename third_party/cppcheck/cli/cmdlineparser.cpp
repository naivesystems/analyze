...
bool CmdLineParser::parseFromArgs(int argc, const char* const argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            else if (std::strcmp(argv[i], "--abspath") == 0)
                mSettings->abspath = true;
            ...
            } else if (std::strcmp(argv[i], "--std=c89") == 0 || std::strcmp(argv[i], "--std=c90") == 0) {
    ...
    if (mSettings->abspath) {
        for (auto& pathName: mPathNames) {
            pathName = Path::getAbsoluteFilePath(pathName);
        }
    }
...
void CmdLineParser::printHelp()
{
    std::cout << "Cppcheck - A tool for static C/C++ code analysis\n"
        ...
        "Options:\n"
        "    --abspath\n"
        "                         Force to use and dump absolute path.\n"
        "    --addon=<addon>\n"
