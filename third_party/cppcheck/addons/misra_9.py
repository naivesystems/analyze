...
class ElementDef:
    ...
    def setInitialized(self, designated=False, positional=False, token=None):
        if designated:
                    ...
                    if isFirstElement and self.token.str == '0' and self.token.next.str == '}':
                        ...
                        if self.ed.valueType.pointer - self.ed.getEffectiveLevel() == 1:
                            ...
                            if(self.root.name != '->' and self.ed.parent.parent != self.root) or (self.root.name == '->' and self.root.children[0] != self.ed.parent):
                                ...
                                self.ed.parent.setInitialized(isDesignated, False, self.token)
                            self.ed.parent.initializeChildren()
                    # deal with array of pointer of record
                    elif self.token.valueType and self.token.valueType.pointer > 0 and self.ed.parent.valueType and self.ed.parent.valueType.type == 'record' and self.token.valueType.pointer == self.ed.parent.valueType.pointer - 1:
                        self.ed.parent.setInitialized(isDesignated)
                        if (self.ed.parent.parent):
                            self.ed = self.ed.parent.parent.getNextChild()
                    else:
                        if self.ed.parent != self.root:
                            ...
                            if self.token.valueType:
                                if child.elementType != 'record' or self.token.valueType.type != 'record' or child.valueType.typeScope != self.token.valueType.typeScope:
                                    self.root.markStuctureViolation(self.token)
                        self.ed.setInitialized(isDesignated, False, self.token)
        ...
        while self.token:
            if self.token.astParent.astOperand1 == self.token and self.token.astParent.astOperand2:
                if self.ed:
                    ...
                    # Exception: Arrays like this `char str[] = "xxx"`
                    # do not require explicit initializer for each element.
                    if self.token.isString and self.ed.parent and self.ed.parent.valueType.pointer == 1:
...
def misra_9_x(self, data, rule, rawTokens = None, analyze=None):
    ...
    for variable in data.variables:
        ...
        if variable.isArray or variable.isClass:
            ...
            if analyze == "misra_cxx" and rule == 313 and not ed.isMisra95Compliant():
                self.reportCXXError(nameToken, 3, 1, 3)

def getElementDef(nameToken, rawTokens = None):
    if not nameToken.variable:
        ed = ElementDef("value", nameToken.str, nameToken.valueType)
    elif nameToken.variable.isArray:
