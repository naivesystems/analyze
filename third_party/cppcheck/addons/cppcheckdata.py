...
class LocationByDetail(Location):
...
class ValueType:
    ...
    def __init__(self, element):
        ...
        if constness:
            ...
            self.constness = 0
        volatility = element.get('valueType-volatility')
        if volatility:
...
class Token:
    ...
    def copy(self, **kwargs):
        ...
        for attr, value in kwargs.items():
            self_copy.__setattr__(attr, value)
        return self_copy
...
class Scope:
    ...
    def setIsBooleanEnumTrue(self):
        self.isBooleanEnum = True
...
class Variable:
    ...
    Attributes:
        ...
        volatility      Variable volatility (same encoding as ValueType::volatility)
    """
    ...
    def __init__(self, element):
        ...
        self.volatility = element.get('volatility')
        if self.volatility:
            self.volatility = int(self.volatility)
    ...
    def __repr__(self):
        attrs = ["Id", "nameTokenId", "typeStartTokenId", "typeEndTokenId",
                 ...
                 "isReference", "isStatic", "constness", "volatility" ]
        return "{}({})".format(
            "Variable",
            ", ".join(("{}={}".format(a, repr(getattr(self, a))) for a in attrs))
        )
...
class TypedefInfo:
    ...
    def __init__(self, element):
        ...
        self.nameTokenId = element.get('nameToken')
        self.isValid = (element.get('isValid') == '1')

    def setId(self, IdMap):
        self.nameToken = IdMap.get(self.nameTokenId)
...
class Configuration:
    ...
    def set_id_map(self, arguments):
        ...
        for variable in arguments:
            variable.setId(IdMap)
        for typedef in self.typedefInfo:
            typedef.setId(IdMap)
