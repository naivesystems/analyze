...
void Tokenizer::simplifyTypedef()
{
    ...
    for (Token *tok = list.front(); tok; tok = tok->next()) {
        ...
        if (tokOffset->str() == "::") {
            ...
            typedefInfo.name = typeDef->str();
            typedefInfo.filename = list.file(typeDef);
            typedefInfo.lineNumber = typeDef->linenr();
            typedefInfo.column = typeDef->column();
            typedefInfo.used = false;
            typedefInfo.nameToken = typeDef;
            typedefInfo.isValid = false;
        ...
        if (tokOffset->isName() && !tokOffset->isKeyword()) {
            ...
            // or a function typedef
                ...
                // function pointer
                    ...
                    addImplicitIntTokenForArgument(argStart, argEnd);
                    tok = argEnd->next();
                }
        ...
        typedefInfo.used = false;
        typedefInfo.nameToken = typeName;
        typedefInfo.isValid = true;
        mTypedefInfo.push_back(typedefInfo);
        ...
        if (!ok) {
            mTypedefInfo.back().isValid = false;
        }
    }
}
...
bool Tokenizer::simplifyTokenList1(const char FileName[])
{
    ...
    // Update typedef info with simplified tokens
    const Token *tok = tokens();
    for (auto iter = mTypedefInfo.begin() ; iter != mTypedefInfo.end(); ++iter) {
        while(tok->linenr() != iter->lineNumber || tok->column() != iter->column) tok = tok->next();
        assert(tok->str() == iter->name);
        iter->nameToken = const_cast<Token*>(tok);
    }
    ...
    if (!mTypedefInfo.empty()) {
        ...
        for (const TypedefInfo &typedefInfo: mTypedefInfo) {
            out << "    <info"
                ...
                << " nameToken=\"" << (typedefInfo.nameToken) << "\""
                << " isValid=\"" << (typedefInfo.isValid) << "\""
                << "/>" << std::endl;
        }
        out << "  </typedef-info>" << std::endl;
    }
...
bool Tokenizer::addImplicitIntTokenForArgument(Token *argStart, Token *argEnd)
{
    bool ret = false;

    for (Token *tok = argStart; tok != argEnd; tok = tok->next()) {
        if (Token::Match(tok, "const|extern *|&|%name%")) {
            tok->insertToken("int");
