...
void TokenList::createTokens(simplecpp::TokenList&& tokenList)
{
    ...
    for (const simplecpp::Token *tok = tokenList.cfront(); tok;) {
        ...
        auto revealedLocation = tok->location.revealRealFileIndexAndLine();
        auto fileIndex = revealedLocation.first;
        auto line = revealedLocation.second;
        mTokensFrontBack.back->fileIndex(fileIndex);
        mTokensFrontBack.back->linenr(line);
