...
int ClangTool::run(ToolAction *Action) {
  ...
  for (llvm::StringRef File : AbsolutePaths) {
    ...
    for (CompileCommand &CompileCommand : CompileCommandsForFile) {
      ...
      // When CWD changed, the cached file status with relative path might be invalid.
      // Currently refresh all of them to keep correct.
      // FIXME: Should use abs path as key.
      Files.reset();
