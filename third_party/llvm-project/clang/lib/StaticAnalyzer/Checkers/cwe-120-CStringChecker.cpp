...
class CStringChecker : public Checker< eval::Call,
  ...
  mutable std::unique_ptr<BugType> BT_Null, BT_Bounds, BT_Overlap,
      BT_NotCString, BT_AdditionOverflow, BT_UninitRead, BT_InsecureBufferCopy,
      BT_StackBufferOverflow, BT_HeapBufferOverflow;
...
public:
  ...
  struct CStringChecksFilter {
    ...
    bool CheckCStringInsecureBufferCopy = false;
    bool CheckCStringStackBufferOverflow = false;
    bool CheckCStringHeapBufferOverflow = false;
    ...
    CheckerNameRef CheckNameCStringInsecureBufferCopy;
    CheckerNameRef CheckNameCStringStackBufferOverflow;
    CheckerNameRef CheckNameCStringHeapBufferOverflow;
  };
  ...
  void emitNotCStringBug(CheckerContext &C, ProgramStateRef State,
                         const Stmt *S, StringRef WarningMsg) const;
  void emitInsecureBufferCopyBug(CheckerContext &C, ProgramStateRef State,
                         const Stmt *S, StringRef WarningMsg) const;
  void emitStackBufferOverflowBug(CheckerContext &C, ProgramStateRef State,
                         const Stmt *S, StringRef WarningMsg) const;
  void emitHeapBufferOverflowBug(CheckerContext &C, ProgramStateRef State,
                         const Stmt *S, StringRef WarningMsg) const;
...
ProgramStateRef CStringChecker::CheckLocation(CheckerContext &C,
  ...
  if (StOutBound && !StInBound) {
    ...
    return nullptr;
  } else if (StOutBound && Filter.CheckCStringInsecureBufferCopy) {
    ErrorMessage Message = createOutOfBoundErrorMsg(CurrentFunctionDescription, Access);
    emitInsecureBufferCopyBug(C, StOutBound, Buffer.Expression, Message);
    return nullptr;
  } else if (StOutBound && Filter.CheckCStringStackBufferOverflow &&
             ER->getSuperRegion()->hasStackStorage()) {
    ErrorMessage Message = createOutOfBoundErrorMsg(CurrentFunctionDescription, Access);
    emitStackBufferOverflowBug(C, StOutBound, Buffer.Expression, Message);
    return nullptr;
  } else if (StOutBound && Filter.CheckCStringHeapBufferOverflow &&
             !ER->getSuperRegion()->hasStackStorage()) {
    ErrorMessage Message = createOutOfBoundErrorMsg(CurrentFunctionDescription, Access);
    emitHeapBufferOverflowBug(C, StOutBound, Buffer.Expression, Message);
    return nullptr;
  }
...
// added to support CWE-120
void CStringChecker::emitInsecureBufferCopyBug(CheckerContext &C, ProgramStateRef State,
                                               const Stmt *S,
                                               StringRef WarningMsg) const {
  if (ExplodedNode *N = C.generateNonFatalErrorNode(State)) {
    if (!BT_InsecureBufferCopy)
      BT_InsecureBufferCopy.reset(new BuiltinBug(
        Filter.CheckNameCStringInsecureBufferCopy, "API",
        "Buffer Copy without Checking Size of Input ('Classic Buffer Overflow')"
      ));

    auto Report = std::make_unique<PathSensitiveBugReport>(*BT_InsecureBufferCopy, WarningMsg, N);
    Report->addRange(S->getSourceRange());
    C.emitReport(std::move(Report));
  }
}

// added to support CWE-121
void CStringChecker::emitStackBufferOverflowBug(CheckerContext &C, ProgramStateRef State,
                                               const Stmt *S,
                                               StringRef WarningMsg) const {
  if (ExplodedNode *N = C.generateNonFatalErrorNode(State)) {
    if (!BT_StackBufferOverflow)
      BT_StackBufferOverflow.reset(new BuiltinBug(
        Filter.CheckNameCStringStackBufferOverflow, "API",
        "Stack-based Buffer Overflow"
      ));

    auto Report = std::make_unique<PathSensitiveBugReport>(*BT_StackBufferOverflow, WarningMsg, N);
    Report->addRange(S->getSourceRange());
    C.emitReport(std::move(Report));
  }
}

// added to support CWE-122
void CStringChecker::emitHeapBufferOverflowBug(CheckerContext &C, ProgramStateRef State,
                                               const Stmt *S,
                                               StringRef WarningMsg) const {
  if (ExplodedNode *N = C.generateNonFatalErrorNode(State)) {
    if (!BT_HeapBufferOverflow)
      BT_HeapBufferOverflow.reset(new BuiltinBug(
        Filter.CheckNameCStringHeapBufferOverflow, "API",
        "Stack-based Buffer Overflow"
      ));

    auto Report = std::make_unique<PathSensitiveBugReport>(*BT_HeapBufferOverflow, WarningMsg, N);
    Report->addRange(S->getSourceRange());
    C.emitReport(std::move(Report));
  }
}
...
void CStringChecker::evalMemchr(CheckerContext &C, const CallExpr *CE) const {
  ...
  CurrentFunctionDescription = "Locate character in block of memory";
  ...
  std::tie(stateZeroSize, stateNonZeroSize) =
      assumeZero(C, State, sizeVal, sizeTy);
  // If the size can be zero, the result will be NULL in that case, and we don't
...
void ento::registerCWE120CStringModeling(CheckerManager &Mgr) {
  Mgr.registerChecker<CStringChecker>();
}

bool ento::shouldRegisterCWE120CStringModeling(const CheckerManager &mgr) {
  return true;
}
...
REGISTER_CHECKER(CStringInsecureBufferCopy)
REGISTER_CHECKER(CStringStackBufferOverflow)
REGISTER_CHECKER(CStringHeapBufferOverflow)
