...
//=== misrac-2012-d4_14-TaintCheck.cpp -------------------------*- C++ -*--===//
//
// According to dir 4.14, The validity of values received from external sources
// shall be checked
// This can occur:
//  - when functions read from a file
//  - when functions read from an environment variable
//  - when functions resulting from user input;
//  - when functions received over a communications channel
// This checker is modified on top of the original clang static checker
// GenericTaintChecker.cpp. A new check on BranchCondition and some built-in functions
// were moved from propagation into source
//
// The original checker was in charge of examing the taintness of arguments between
// function calls.  A new check on the branch condition is added  for ensuring
//
// Add a new function:
// - checkBranchCondition
//   The checker examines the condition and remove the taintness for the taint operand
//
//===----------------------------------------------------------------------===//
...
namespace {
class TaintMisraChecker : public Checker<check::PreCall, check::PostCall, check::BranchCondition, check::PreStmt<BinaryOperator>, check::PreStmt<ArraySubscriptExpr>> {
public:
  static void *getTag() {
    static int Tag;
    return &Tag;
  ...
  void checkPreStmt(const BinaryOperator *Bin, CheckerContext &C) const;
  void checkPreStmt(const ArraySubscriptExpr *Arr, CheckerContext &C) const;
...
private:
  ...
  void visitExpr(const Expr*Exp, CheckerContext &C) const;
  ...
  static constexpr llvm::StringLiteral MsgMisra =
      "violation of misra_c_2012: dir_4_14";
  bool checkFormatString(const CallEvent &Call, CheckerContext &C) const;
...
const unsigned TaintMisraChecker::ReturnValueIndex;
const unsigned TaintMisraChecker::InvalidArgIndex;

// FIXME: these lines can be removed in C++17
constexpr llvm::StringLiteral TaintMisraChecker::MsgSanitizeSystemArgs;
constexpr llvm::StringLiteral TaintMisraChecker::MsgTaintedBufferSize;
constexpr llvm::StringLiteral TaintMisraChecker::MsgCustomSink;
constexpr llvm::StringLiteral TaintMisraChecker::MsgMisra;
} // end of anonymous namespace

using TaintConfig = TaintMisraChecker::TaintConfiguration;
...
template <> struct MappingTraits<TaintConfig::Propagation> {
  static void mapping(IO &IO, TaintConfig::Propagation &Propagation) {
    ...
    IO.mapOptional("VariadicType", Propagation.VarType,
                   TaintMisraChecker::VariadicType::None);
    IO.mapOptional("VariadicIndex", Propagation.VarIndex,
                   TaintMisraChecker::InvalidArgIndex);
  }
};

template <> struct ScalarEnumerationTraits<TaintMisraChecker::VariadicType> {
  static void enumeration(IO &IO, TaintMisraChecker::VariadicType &Value) {
    IO.enumCase(Value, "None", TaintMisraChecker::VariadicType::None);
    IO.enumCase(Value, "Src", TaintMisraChecker::VariadicType::Src);
    IO.enumCase(Value, "Dst", TaintMisraChecker::VariadicType::Dst);
  }
};
...
TaintMisraChecker::ArgVector
TaintMisraChecker::convertToArgVector(CheckerManager &Mgr,
...
void TaintMisraChecker::parseConfiguration(CheckerManager &Mgr,
                                             const std::string &Option,
                                             TaintConfiguration &&Config) {
  for (auto &P : Config.Propagations) {
    TaintMisraChecker::CustomPropagations.emplace(
        P.Name,
...
template <typename T>
auto TaintMisraChecker::findFunctionInConfig(const ConfigDataMap<T> &Map,
                                               const FunctionData &FData) {
...
TaintMisraChecker::TaintPropagationRule
TaintMisraChecker::TaintPropagationRule::getTaintPropagationRule(
  ...
  TaintPropagationRule Rule =
      llvm::StringSwitch<TaintPropagationRule>(FData.FullName)
          ...
          .Case("fgetc", {{}, {ReturnValueIndex}})
          .Case("fgetln", {{}, {ReturnValueIndex}})
          .Case("fgets", {{}, {0, ReturnValueIndex}})
          .Case("fscanf", {{}, {}, VariadicType::Dst, 2})
          .Case("sscanf", {{}, {}, VariadicType::Dst, 2})
          .Case("getc", {{}, {ReturnValueIndex}})
          .Case("getc_unlocked", {{}, {ReturnValueIndex}})
          .Case("getdelim", {{}, {0}})
          .Case("getline", {{}, {0}})
          .Case("getw", {{}, {ReturnValueIndex}})
          .Case("pread", {{}, {1}})
          .Case("read", {{}, {1}})
          .Case("readlink", {{}, {ReturnValueIndex}})
          .Case("fread", {{}, {0}})
...
void TaintMisraChecker::checkPreCall(const CallEvent &Call,
                                       CheckerContext &C) const {
...
void TaintMisraChecker::checkPostCall(const CallEvent &Call,
                                        CheckerContext &C) const {
...
// This function is used to recursively visit expression
// In case the expression is taint then it issues the report
void TaintMisraChecker::visitExpr(const Expr*Exp, CheckerContext &C) const{
  if(generateReportIfTainted(Exp, MsgMisra, C))
    return;
  if(auto *Bin = dyn_cast<BinaryOperator>(Exp)){
  ...
  if(auto *Unary = dyn_cast<UnaryOperator>(Exp)){
    Expr *Operand = Unary -> getSubExpr();
    visitExpr(Operand, C);
    return;
  }
}

void TaintMisraChecker::checkBranchCondition(const Stmt *CS, CheckerContext &C) const {
  ProgramStateRef State = C.getState();
  const ParentMap &Parents = C.getLocationContext()->getParentMap();
  const Stmt *Parent = Parents.getParent(CS);
  // The loop is used to propagte to the nearest enclosing scope for the branch condition
  // Since the parent for expression like while(a && b)
  // The parent for expression a/b is the binary operator not the while loop
  // This loop is introduced to find the loop
  while(!(Parent == nullptr || dyn_cast<WhileStmt>(Parent) || dyn_cast<ForStmt>(Parent) || dyn_cast<IfStmt>(Parent)))
    Parent = Parents.getParent(Parent);
  if(dyn_cast<WhileStmt>(Parent) || dyn_cast<ForStmt>(Parent)){
    if(auto* Exp = dyn_cast<Expr>(CS))
      visitExpr(Exp, C);
    return;
  }
  
  if(dyn_cast<IfStmt>(Parent)){
    if(auto *BinOp = dyn_cast<BinaryOperator>(CS)){
      Expr *LHS = BinOp -> getLHS();
      Expr *RHS = BinOp -> getRHS();
      if(taint::isTainted(State, LHS, C.getLocationContext())){
        State = taint::removeTaint(State, C.getSVal(LHS));
        C.addTransition(State);
      }
      if(taint::isTainted(State, RHS, C.getLocationContext())){
        State = taint::removeTaint(State, C.getSVal(RHS));
        C.addTransition(State);
      }
      return;
    }
    if(auto *UnaryOp = dyn_cast<UnaryOperator>(CS)){
      Expr *Operand = UnaryOp -> getSubExpr();
      if(taint::isTainted(State, Operand, C.getLocationContext())){
        State = taint::removeTaint(State, C.getSVal(Operand));
        C.addTransition(State);
      }
      return;
    }
      ...
      if(auto *Exp = dyn_cast<Expr>(CS)){
        if(taint::isTainted(State, Exp, C.getLocationContext())){
          State = taint::removeTaint(State, C.getSVal(Exp));
          C.addTransition(State);
        }
        return;
      }
...
void TaintMisraChecker::checkPreStmt(const BinaryOperator *Bin, CheckerContext &C) const {
  BinaryOperator::Opcode Op = Bin->getOpcode();
  if(Op == BO_Div || Op == BO_Rem || Op == BO_DivAssign || Op == BO_RemAssign){
    Expr *LHS = Bin -> getLHS();
    Expr *RHS = Bin -> getRHS();
    ProgramStateRef State = C.getState();
    generateReportIfTainted(LHS, MsgMisra, C);
    generateReportIfTainted(RHS, MsgMisra, C);
    return;
  }
  return;
}

void TaintMisraChecker::checkPreStmt(const ArraySubscriptExpr *Arr, CheckerContext &C) const {
  const Expr *Idx = Arr->getIdx();
  ProgramStateRef State = C.getState();
  generateReportIfTainted(Idx, MsgMisra, C);
  return;
}

void TaintMisraChecker::printState(raw_ostream &Out, ProgramStateRef State,
                                     const char *NL, const char *Sep) const {
  printTaint(State, Out, NL, Sep);
}

bool TaintMisraChecker::addSourcesPre(const CallEvent &Call,
                                        const FunctionData &FData,
                                        CheckerContext &C) const {
...
bool TaintMisraChecker::addFiltersPre(const CallEvent &Call,
                                        const FunctionData &FData,
                                        CheckerContext &C) const {
...
bool TaintMisraChecker::propagateFromPre(const CallEvent &Call,
                                           CheckerContext &C) {
  ProgramStateRef State = C.getState();
...
bool TaintMisraChecker::checkPre(const CallEvent &Call,
                                   const FunctionData &FData,
                                   CheckerContext &C) const {
  if (checkFormatString(Call, C))
    return true;
...
Optional<SVal> TaintMisraChecker::getPointeeOf(CheckerContext &C,
                                                 const Expr *Arg) {
  ProgramStateRef State = C.getState();
...
ProgramStateRef
TaintMisraChecker::TaintPropagationRule::process(const CallEvent &Call,
                                                   CheckerContext &C) const {
  ProgramStateRef State = C.getState();
...
bool TaintMisraChecker::TaintPropagationRule::postSocket(
...
bool TaintMisraChecker::isStdin(const Expr *E, CheckerContext &C) {
  ProgramStateRef State = C.getState();
...
bool TaintMisraChecker::generateReportIfTainted(const Expr *E, StringRef Msg,
                                                  CheckerContext &C) const {
  assert(E);
...
bool TaintMisraChecker::checkFormatString(
  ...
  // warn.
  bool res = false;
  for(unsigned int i = ArgNum; i < Call.getNumArgs(); ++i) {
    if(generateReportIfTainted(Call.getArgExpr(i), MsgMisra, C)) {
        res = true;
    }
  }
  return res;
...
bool TaintMisraChecker::checkSystemCall(const CallEvent &Call, StringRef Name,
                                          CheckerContext &C) const {
...
bool TaintMisraChecker::checkTaintedBufferSize(const CallEvent &Call,
                                                 CheckerContext &C) const {
...
bool TaintMisraChecker::checkCustomSinks(const CallEvent &Call,
                                           const FunctionData &FData,
                                           CheckerContext &C) const {
  ...
  const auto &Value = It->second;
  const TaintMisraChecker::ArgVector &Args = Value.second;
  for (unsigned ArgNum : Args) {
    if (ArgNum >= Call.getNumArgs())
      continue;
...
void ento::registerTaintMisraChecker(CheckerManager &Mgr) {
  auto *Checker = Mgr.registerChecker<TaintMisraChecker>();
...
bool ento::shouldRegisterTaintMisraChecker(const CheckerManager &mgr) {
  return true;
}
