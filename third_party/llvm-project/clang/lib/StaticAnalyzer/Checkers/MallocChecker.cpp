...
#define CHECK_FN(NAME)                                                         \
  ...
  CallDescriptionMap<CheckFn> AllocatingMemFnMap{
      ...
      {{"aligned_alloc", 2}, &MallocChecker::checkAlignedAlloc},
  ...
  static ProgramStateRef AlignedAllocMem(CheckerContext &C, const CallEvent &Call,
                                   ProgramStateRef State);
...
void MallocChecker::checkAlignedAlloc(const CallEvent &Call,
                                    CheckerContext &C) const {
  ProgramStateRef State = C.getState();
  State = AlignedAllocMem(C, Call, State);
...
ProgramStateRef MallocChecker::AlignedAllocMem(CheckerContext &C,
                                         const CallEvent &Call,
                                         ProgramStateRef State) {
  if (!State)
    return nullptr;
  ...
  return MallocMemAux(C, Call, Call.getArgExpr(1), UndefinedVal(), State, AF_Malloc);
}
