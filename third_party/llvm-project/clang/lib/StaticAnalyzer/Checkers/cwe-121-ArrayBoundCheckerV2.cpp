...
//=== cwe-121-ArrayBoundCheckerV2.cpp ---------------------=-----*- C++ -*-===//
//
...
public:
  bool ShouldCheckStackArray;
  bool ShouldCheckHeapArray;
...
void ArrayBoundCheckerV2::checkLocation(SVal location, bool isLoad,
  ...
  if (!rawOffset.getRegion())
    return;
  if (rawOffset.getRegion()->getSuperRegion()->hasStackStorage()
    && !ShouldCheckStackArray)
    return;
  if (!rawOffset.getRegion()->getSuperRegion()->hasStackStorage()
    && !ShouldCheckHeapArray)
    return;
...
void ArrayBoundCheckerV2::reportOOB(
  ...
  if (!BT)
    BT.reset(new BuiltinBug(CheckName, "Out-of-bound access"));
...
void ento::registerArrayBoundBase(CheckerManager &mgr) {
  mgr.registerChecker<ArrayBoundCheckerV2>();
}

bool ento::shouldRegisterArrayBoundBase(const CheckerManager &mgr) {
  return true;
}

void ento::registerStackArrayBoundChecker(CheckerManager &mgr) {
  ArrayBoundCheckerV2 *checker = mgr.getChecker<ArrayBoundCheckerV2>();
  checker->ShouldCheckStackArray = true;
  checker->CheckName = mgr.getCurrentCheckerName();
}

bool ento::shouldRegisterStackArrayBoundChecker(const CheckerManager &mgr) {
  return true;
}

void ento::registerHeapArrayBoundChecker(CheckerManager &mgr) {
  ArrayBoundCheckerV2 *checker = mgr.getChecker<ArrayBoundCheckerV2>();
  checker->ShouldCheckHeapArray = true;
  checker->CheckName = mgr.getCurrentCheckerName();
}

bool ento::shouldRegisterHeapArrayBoundChecker(const CheckerManager &mgr) {
  return true;
}
