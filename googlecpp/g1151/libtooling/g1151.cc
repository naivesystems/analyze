...
#include "googlecpp/g1151/libtooling/lib.h"
...
llvm::cl::opt<std::string> optional_info_file_opt(
    "OptionalInfoFile",
    llvm::cl::desc("The name of temporary file used in googlecpp/g1151"),
    llvm::cl::cat(ns_libtooling_checker));

int g1151(int argc, char** argv) {
  // Args like `optional-info-file=(filename)` could assign the name of tmp
  // file. So if it needs to modify the file, just pass it like
  // `optional-info-file=(filename)` to the command line.
  ...
  googlecpp::g1151::libtooling::Checker checker;
  checker.Init(&all_results, optional_info_file_opt.getValue());
...
podman_image::bigmain::SuffixRule _("googlecpp/g1151",
                                    googlecpp::g1151::libtooling::g1151);
