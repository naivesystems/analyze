...
#include "googlecpp/g1149/libtooling/lib.h"
...
int g1149(int argc, char** argv) {
  ...
  googlecpp::g1149::libtooling::Checker checker;
  checker.Init(&all_results);
  int status = tool.run(&checker);
  LOG(INFO) << "ppcallbacks status: " << status;
  status = tool.run(
...
podman_image::bigmain::SuffixRule _("googlecpp/g1149",
                                    googlecpp::g1149::libtooling::g1149);
