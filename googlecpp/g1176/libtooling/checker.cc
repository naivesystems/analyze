...
void ReportError(string path, int line_number, ResultsList* results_list) {
  string error_message =
      "Do not use user-defined literals by operator overloading";
 ...
 public:
  ...
  void run(const ast_matchers::MatchFinder::MatchResult& result) override {
    if (const auto* function_decl =
      ...
      if (DeclarationName::CXXLiteralOperatorName ==
          function_decl->getDeclName().getNameKind()) {
        ReportError(misra::libtooling_utils::GetFilename(function_decl,
