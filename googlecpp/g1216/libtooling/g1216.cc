...
#include "googlecpp/g1216/libtooling/lib.h"
...
int g1216(int argc, char** argv) {
  ...
  googlecpp::g1216::libtooling::ActionFactory action_factory;
  analyzer::proto::ResultsList all_results;

  action_factory.Init(&all_results);
  int status = tool.run(&action_factory);
  LOG(INFO) << "libtooling status: " << status;
  if (misra::proto_util::GenerateProtoFile(all_results, results_path).ok()) {
    LOG(INFO) << "g1216 check done";
  }
  return 0;
}
}  // namespace libtooling
...
podman_image::bigmain::SuffixRule _("googlecpp/g1216",
                                    googlecpp::g1216::libtooling::g1216);
