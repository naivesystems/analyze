...
func Analyze(srcdir string, opts *options.CheckOptions) (*pb.ResultsList, error) {
	reportJson, err := runner.RunCSA(srcdir, "-analyzer-checker=core.DivideZero", opts)
	if err != nil {
	...
	for _, run := range reportJson.Runs {
		for _, resultsContent := range run.Results {
			if resultsContent.RuleId == "core.DivideZero" {
				result := &pb.Result{}
				...
				result.ErrorMessage = "The right hand operand of the integer division or remainder operators shall not be equal to zero."
