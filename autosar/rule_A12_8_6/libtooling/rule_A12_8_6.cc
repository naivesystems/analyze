...
#include "autosar/rule_A12_8_6/libtooling/lib.h"
...
int rule_A12_8_6(int argc, char** argv) {
  ...
  autosar::rule_A12_8_6::libtooling::Checker checker;
  // first tranverse to collect all base classes
  checker.InitCollectBasesCallback(&all_results);
  int status1 =
      tool.run(newFrontendActionFactory(checker.GetMatchFinder1()).get());
  // second tranverse to check all base classes if they against the rule
  checker.InitCheckBasesCallback();
  int status2 =
      tool.run(newFrontendActionFactory(checker.GetMatchFinder2()).get());
  int status = status1 > status2 ? status1 : status2;
  LOG(INFO) << "libtooling status: " << status;
  if (misra::proto_util::GenerateProtoFile(all_results, results_path).ok()) {
    LOG(INFO) << "rule A12 8 6 check done";
  }
  return 0;
}
}  // namespace libtooling
...
podman_image::bigmain::SuffixRule _(
    "autosar/rule_A12_8_6", autosar::rule_A12_8_6::libtooling::rule_A12_8_6);
}  // namespace
