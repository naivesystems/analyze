...
#include "autosar/rule_A12_0_1/libtooling/checker.h"
...
class Callback : public ast_matchers::MatchFinder::MatchCallback {
 public:
  void Init(analyzer::proto::ResultsList* results_list,
    ...
    finder->addMatcher(
        cxxRecordDecl(anyOf(has(cxxDestructorDecl()),
                            hasMethod(isCopyAssignmentOperator()),
                            hasMethod(isMoveAssignmentOperator())),
                      unless(allOf(has(cxxDestructorDecl()),
  ...
  void run(const ast_matchers::MatchFinder::MatchResult& result) {
    ...
    if (crd)
      ReportError(
          misra::libtooling_utils::GetFilename(crd, result.SourceManager),
          misra::libtooling_utils::GetLine(crd, result.SourceManager),
          results_list_);
  }
