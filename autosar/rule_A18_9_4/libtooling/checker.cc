...
class Callback : public ast_matchers::MatchFinder::MatchCallback {
 public:
  void Init(analyzer::proto::ResultsList* results_list,
    ...
    finder->addMatcher(
        callExpr(
            callee(functionDecl(hasName("std::forward"))),
            hasArgument(0, declRefExpr().bind("forward_arg")),
            hasAncestor(
  ...
  void run(const MatchFinder::MatchResult& result) override {
    ...
    const DeclRefExpr* forward_arg =
        result.Nodes.getNodeAs<DeclRefExpr>("forward_arg");
    const Decl* forwarded_decl = forward_arg->getDecl();
