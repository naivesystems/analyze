...
constexpr char kEnumDeclString[] = "enumDeclString";
...
void Callback::Init(ResultsList* results_list, MatchFinder* finder) {
  ...
  finder->addMatcher(enumDecl().bind(kEnumDeclString), this);
}

void Callback::run(const MatchFinder::MatchResult& result) {
  const EnumDecl* enum_decl = result.Nodes.getNodeAs<EnumDecl>(kEnumDeclString);
  std::string path =
