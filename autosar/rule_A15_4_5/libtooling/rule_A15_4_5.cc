...
#include "autosar/rule_A15_4_5/libtooling/lib.h"
...
int rule_A15_4_5(int argc, char** argv) {
  ...
  autosar::rule_A15_4_5::libtooling::CommentChecker comment_checker(
      &all_results);
  tool.appendArgumentsAdjuster(
      clang::tooling::getInsertArgumentAdjuster("-fparse-all-comments"));
  int comment_status = tool.run(&comment_checker);
  autosar::rule_A15_4_5::libtooling::Checker checker;
  checker.Init(&all_results);
  int status = tool.run(
      tooling::newFrontendActionFactory(checker.GetMatchFinder()).get());
  int result_status = (status | comment_status);
  checker.Report();
  ...
  if (misra::proto_util::GenerateProtoFile(all_results, results_path).ok()) {
    LOG(INFO) << "rule_A15_4_5 check done";
  }
  return 0;
}
}  // namespace libtooling
...
podman_image::bigmain::SuffixRule _(
    "autosar/rule_A15_4_5", autosar::rule_A15_4_5::libtooling::rule_A15_4_5);
}  // namespace
