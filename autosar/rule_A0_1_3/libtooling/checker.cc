...
#include "autosar/rule_A0_1_3/libtooling/checker.h"
...
void Callback::Init(ResultsList* results_list, MatchFinder* finder) {
  ...
  // Match function declarations in anonymous namespaces.
  finder->addMatcher(functionDecl(hasAncestor(namespaceDecl(isAnonymous())))
                         .bind(kFuncDeclString),
                     this);
  finder->addMatcher(functionDecl(isStaticStorageClass()).bind(kFuncDeclString),
                     this);
  finder->addMatcher(cxxMethodDecl(isPrivate()).bind(kFuncDeclString), this);
}

void Callback::run(const MatchFinder::MatchResult& result) {
  const FunctionDecl* func_decl =
      result.Nodes.getNodeAs<FunctionDecl>(kFuncDeclString);
  ;
  if (func_decl && func_decl->isFirstDecl() &&
      !func_decl->hasAttr<UnusedAttr>() && !func_decl->isUsed()) {
    std::string path =
