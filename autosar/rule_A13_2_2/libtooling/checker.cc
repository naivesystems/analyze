...
#include "autosar/rule_A13_2_2/libtooling/checker.h"
...
const std::unordered_set<OverloadedOperatorKind> kBinaryFormatOperatorSet = {
    OO_Plus,    OO_Minus, OO_Star, OO_Slash,
    OO_Percent, OO_Caret, OO_Amp,  OO_Pipe};
...
void Callback::run(const MatchFinder::MatchResult& result) {
  ...
  if (kBinaryFormatOperatorSet.find(function_decl->getOverloadedOperator()) !=
          kBinaryFormatOperatorSet.end() &&
      ...
      (function_decl->getReturnType()->isReferenceType() ||
       function_decl->getReturnType()->isPointerType() ||
       function_decl->getReturnType().isConstQualified())) {
