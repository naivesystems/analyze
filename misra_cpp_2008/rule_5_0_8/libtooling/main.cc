...
#include "misra_cpp_2008/rule_5_0_8/libtooling/lib.h"
...
static cl::opt<std::string> rule_name("rule-name",
                                      cl::desc("select which rule to check"),
                                      cl::init("rule-5-0-8"));

namespace misra_cpp_2008 {
namespace rule_5_0_8 {
namespace libtooling {
int rule_5_0_8(int argc, char** argv) {
  ...
  misra_cpp_2008::rule_5_0_8::libtooling::Checker checker;
  analyzer::proto::ResultsList all_results;

  checker.Init(rule_name, &all_results);
  int status =
  ...
  if (misra::proto_util::GenerateProtoFile(all_results, results_path).ok()) {
    LOG(INFO) << "rule 5.0.8 check done";
  }
  return 0;
}
}  // namespace libtooling
...
podman_image::bigmain::SuffixRule _(
    "misra_cpp_2008/rule_5_0_8",
    misra_cpp_2008::rule_5_0_8::libtooling::rule_5_0_8);
