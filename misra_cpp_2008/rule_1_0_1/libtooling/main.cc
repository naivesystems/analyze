...
#include "misra_cpp_2008/rule_1_0_1/libtooling/lib.h"
...
int rule_1_0_1(int argc, char** argv) {
  ...
  misra_cpp_2008::rule_1_0_1::libtooling::Checker checker;
  analyzer::proto::ResultsList all_results;

  checker.Init(&all_results);
  tool.setDiagnosticConsumer(&checker);
  tool.appendArgumentsAdjuster(getInsertArgumentAdjuster("-std=c++03"));
  int status =
  ...
  if (misra::proto_util::GenerateProtoFile(all_results, results_path).ok()) {
    LOG(INFO) << "rule 1.0.1 check done";
  }
  return 0;
}
}  // namespace libtooling
...
podman_image::bigmain::SuffixRule _(
    "misra_cpp_2008/rule_1_0_1",
    misra_cpp_2008::rule_1_0_1::libtooling::rule_1_0_1);
