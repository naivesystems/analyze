...
#include "misra_cpp_2008/rule_7_3_5/libtooling/lib.h"
...
int rule_7_3_5(int argc, char** argv) {
  ...
  misra_cpp_2008::rule_7_3_5::libtooling::NamespaceChecker namespace_checker;
  misra_cpp_2008::rule_7_3_5::libtooling::UsingChecker using_checker;
  analyzer::proto::ResultsList all_results;

  namespace_checker.Init(&all_results);
  int status = tool.run(
      newFrontendActionFactory(namespace_checker.GetMatchFinder()).get());
  LOG(INFO) << "NamespaceChecker status: " << status;
  using_checker.Init(&all_results);
  status =
      tool.run(newFrontendActionFactory(using_checker.GetMatchFinder()).get());
  LOG(INFO) << "UsingChecker status: " << status;
  if (misra::proto_util::GenerateProtoFile(all_results, results_path).ok()) {
    LOG(INFO) << "rule 7_3_5 check done";
  }
  return 0;
}
}  // namespace libtooling
...
podman_image::bigmain::SuffixRule _(
    "misra_cpp_2008/rule_7_3_5",
    misra_cpp_2008::rule_7_3_5::libtooling::rule_7_3_5);
